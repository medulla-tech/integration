...
ANSIBLE_COMMUNITY_URL='https://resources.siveo.net/quickserverinstall/community-general-9.1.0.tar.gz'
...
disable_ipv6() {
    ...
    # Disable IPv6
    #
    colored_echo blue "Disabling IPv6..."
    echo '# Disabling the IPv6' >> /etc/sysctl.conf
    echo 'net.ipv6.conf.all.disable_ipv6 = 1' >> /etc/sysctl.conf
    echo 'net.ipv6.conf.default.disable_ipv6 = 1' >> /etc/sysctl.conf
    echo 'net.ipv6.conf.lo.disable_ipv6 = 1' >> /etc/sysctl.conf
    sysctl -p
    colored_echo green "Disabling IPv6... DONE"
}
...
define_minimum_vars() {
    ...
    for IP in ${IP_ADDRESSES}; do
        if [[ $IP =~ ^10\.|^172\.(1[6-9]|2[0-9]|3[01])\.|^192\.168\.|^169\.254\.|^127\. ]]; then
            INTERFACES+=($(ip -o addr | grep ${IP} | awk '{print $2}'))
        else
            PUBLIC_IP=${IP}
        fi
    done
    if [ ${#INTERFACES[@]} -eq 1 ]; then
        ...
        display_error_message "The server has more than one interface: ${INTERFACES[@]}. The installer cannot figure out which one to use"
        exit 1
    else
        display_error_message "No interface with an IP address was found. Medulla needs an interface for connecting to its clients"
        exit 1
    fi
    ...
    PLAYBOOK_URL='https://github.com/medulla-tech/integration/archive/849d05f.tar.gz'
...
install_ansible() {
    ...
    local CMD="apt install -y ansible"
    eval ${CMD}
    if [ $? -ne 0 ]; then
        display_error_message "Ansible could not be installed" "${CMD}"
        exit 1
    fi
    local CMD="wget -c ${ANSIBLE_COMMUNITY_URL}"
    eval ${CMD}
    if [ $? -ne 0 ]; then
        display_error_message "Ansible community.general could not be downloaded" "${CMD}"
        exit 1
    fi
    local CMD="ansible-galaxy collection install community-general-*.tar.gz"
    eval ${CMD}
    if [ $? -ne 0 ]; then
        display_error_message "Ansible community.general collection could not be installed" "${CMD}"
        exit 1
    fi
    local CMD="apt install -y python3-passlib python3-bcrypt python3-pymysql xmlstarlet python3-lxml python3-selinux python3-pyldap python3-openssl"
    eval ${CMD}
    if [ $? -ne 0 ]; then
        display_error_message "Ansible could not be installed" "${CMD}"
        exit 1
...
reconfigure_network() {
    # 
    # Reconfigure network for a static IP address
    # 
    VM_IP=$(ip -o -f inet addr show ${INTERFACE} | awk '{print $4}')
    VM_GATEWAY=$(ip route | awk '/default/ {print $3}')
    VM_DNS=$(grep ^nameserver /etc/resolv.conf | awk '{print $2}')
    sed -i "s/iface ${INTERFACE} inet .*$/iface ${INTERFACE} inet static\naddress $VM_IP\ngateway $VM_GATEWAY\ndns-nameservers $VM_DNS/" /etc/network/interfaces
